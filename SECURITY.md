...
5. **数据落盘。** SQLite 数据库、执行日志(含 stdout/stderr)、失败截图都明文存放在 `data_dir`。日志里可能包含工具打印的账号昵称等信息,分享前自查。已结束的执行日志按 `execution_retention_days`(默认 30 天,`0` 关闭)自动清理,至少保留最近 1000 条,避免数据库无限膨胀。
